...
window.addEventListener('load', async () => {
    ...
    // แสดงปุ่ม Toggle เฉพาะ admin/super_admin
...
async function loadClassrooms() {
    ...
    // โหมดครู: กรองเฉพาะห้องที่ตนเองเป็นที่ปรึกษา
    if (!isAdminMode) {
...
async function loadResults(forceClassroomId = null) {
    ...
    // ได้ห้องจาก Tom Select (admin) หรือ parameter (teacher)
...
function renderTable(rows) {
    ...
    eqTable = new DataTable('#eq-table', {
        ...
        scrollX: true, 
        pageLength: 50,
        columnDefs: [{ orderable: false, targets: [10] },{ responsivePriority: 1, targets: -1 }],
    });
}
