...
// ==========================================
// ฟังก์ชันเสริมสำหรับสถิติ
// ==========================================
function getDimIconRIASEC(key) {
    const map = {
        realistic: 'fa-hard-hat',
        investigative: 'fa-flask',
        artistic: 'fa-palette',
        social: 'fa-hand-holding-heart',
        enterprising: 'fa-handshake',
        conventional: 'fa-clipboard-list'
    };
...
async function loadStats(forceClassroomId = null) {
    ...
    let query = db.from('riasec_assessments')
        .select('student_id, classroom_id, score_realistic, score_investigative, score_artistic, score_social, score_enterprising, score_conventional')
    ...
    const { data: riasecs, error: riasecErr } = await query;
    if (riasecErr) console.error('loadStats error:', riasecErr);

    let visibleStudentIds = [];
    if (!isAdminMode) {
        const roomIds = allClassrooms.map(r => r.id);
        if (roomIds.length === 0) {
            const emptyDimStats = RIASEC_DIMENSIONS.map(d => ({ key: d.key, label: d.label, count: 0, icon: getDimIconRIASEC(d.key) }));
    ...
    const filteredAssessments = (riasecs || []).filter(m => visibleStudentIds.includes(m.student_id));
    const assessedCount = filteredAssessments.length;
    const notAssessedCount = totalStudents - assessedCount;

    const dimKeys = RIASEC_DIMENSIONS.map(d => d.key);
    const topCount = {};
    ...
    const dimStats = RIASEC_DIMENSIONS.map(d => ({
        key: d.key,
        label: d.label,
        count: topCount[d.key] || 0,
        icon: getDimIconRIASEC(d.key)
    }));
...
function renderStatsCards(total, assessed, notAssessed, dimStats) {
    ...
    const colors = ['red', 'blue', 'purple', 'green', 'amber', 'teal'];
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-5 mt-6">
    <div class="glass rounded-2xl shadow-sm p-5">
        ...
        <div class="relative" style="height:240px;">
            <canvas id="riasec-bar-chart"></canvas>
        </div>
    </div>
    <div class="glass rounded-2xl shadow-sm p-5">
        <h3 class="font-bold text-slate-700 text-sm mb-4 flex items-center gap-2">
            <i class="fas fa-chart-pie text-purple-500"></i> สัดส่วนบุคลิกภาพที่โดดเด่น (%)
        </h3>
        <div class="flex items-center gap-4">
            <div class="relative flex-shrink-0" style="width:150px;height:150px;">
                <canvas id="riasec-doughnut-chart"></canvas>
                <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <p class="text-xs text-slate-400 font-bold">รวม</p>
                    <p class="text-xl font-black text-slate-700" id="riasec-doughnut-total">0</p>
                    <p class="text-xs text-slate-400">คน</p>
                </div>
            </div>
            <div class="flex-1 space-y-1 text-xs" style="max-height:220px; overflow-y:auto;" id="riasec-doughnut-legend"></div>
        </div>
    </div>
</div>`;

    container.innerHTML = html;
    requestAnimationFrame(() => renderRIASECCharts(dimStats));
}

function renderRIASECCharts(dimStats) {
    const DIM_COLORS = ['#ef4444', '#3b82f6', '#8b5cf6', '#22c55e', '#f59e0b', '#14b8a6'];
    const labels = dimStats.map(d => d.label);
    const counts = dimStats.map(d => d.count);
    const total = counts.reduce((a, b) => a + b, 0);

    const barCanvas = document.getElementById('riasec-bar-chart');
    if (barCanvas) {
        if (riasecBarChartInstance) { riasecBarChartInstance.destroy(); riasecBarChartInstance = null; }
        const sortedIdx = [...counts.keys()].sort((a, b) => counts[b] - counts[a]);
        riasecBarChartInstance = new Chart(barCanvas.getContext('2d'), {
            type: 'bar',
            data: {
    ...
    const doughnutCanvas = document.getElementById('riasec-doughnut-chart');
    const legendEl = document.getElementById('riasec-doughnut-legend');
    const totalEl = document.getElementById('riasec-doughnut-total');
    if (doughnutCanvas) {
        if (riasecDoughnutChartInstance) { riasecDoughnutChartInstance.destroy(); riasecDoughnutChartInstance = null; }
        ...
        riasecDoughnutChartInstance = new Chart(doughnutCanvas.getContext('2d'), {
            type: 'doughnut',
...
async function loadResults(forceClassroomId = null) {
    ...
    try {
        ...
        if (riasecErr) console.error('loadResults riasec error:', riasecErr);
        (riasecs || []).forEach(e => { riasecMap[e.student_id] = e; });
        ...
        Swal.close();
        allResults = (enrolls || []).map(e => ({ ...e, riasec: riasecMap[e.student_id] || null }));
...
function renderTable(rows) {
    if (riasecTable) { riasecTable.destroy(); riasecTable = null; }
    const tbody = document.getElementById('riasec-tbody');
    if (!tbody) return;
    ...
    for (const r of rows) {
        ...
        if (!riasec) {
            html += `<tr>
                ...
                ${RIASEC_DIMENSIONS.map(() => '<td class="text-center">-</td>').join('')}
        ...
        const dimKeys = RIASEC_DIMENSIONS.map(d => d.key);
        const dimScores = dimKeys.map(key => riasec[`score_${key}`] || 0);
    ...
    try {
        riasecTable = new DataTable('#riasec-table', {
            language: { url: 'https://cdn.datatables.net/plug-ins/2.3.7/i18n/th.json' },
            responsive: true,
            scrollX: true,
            pageLength: 50,
            columnDefs: [{ responsivePriority: 1, targets: -1 }, { orderable: false, targets: [11] }]
        });
    } catch (err) { console.error('DataTable init error:', err); }
}

// ==========================================
// ฟังก์ชันดูผล, แก้ไข, ลบ, ส่งออก
// ==========================================
...
async function openViewResult(studentId) {
    ...
    if (!row) {
        ...
        const { data: riasec } = await db.from('riasec_assessments')
            .select('*')
            .eq('student_id', studentId)
            .eq('academic_year', String(schoolInfo?.current_academic_year))
            .eq('semester', String(schoolInfo?.current_semester))
    ...
    const dims = RIASEC_DIMENSIONS.map(d => ({
        key: d.key,
        label: d.label,
        score: riasec[`score_${d.key}`] || 0,
        max: d.maxScore,
        level: riasec[`level_${d.key}`] || ''
    }));

    const totalScore = riasec.score_total || 0;
    ...
    document.getElementById('vr-body').innerHTML = `
        ...
        ${riasec.note ? `<div class="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-800"><i class="fas fa-note-sticky mr-1"></i><strong>หมายเหตุ:</strong> ${riasec.note}</div>` : ''}
    `;
...
async function openEditForStudent(studentId) {
    ...
    if (!row) {
        ...
        row = { ...enroll, riasec: null, core_students: enroll.core_students };
    }
    const std = row.core_students;
    const riasec = row.riasec || {};
    document.getElementById('edit-student-id').value = studentId;
    document.getElementById('edit-student-name').textContent = `${std?.prefix || ''}${std?.first_name || ''} ${std?.last_name || ''}`;
    document.getElementById('edit-realistic').value = riasec.score_realistic || 0;
    document.getElementById('edit-investigative').value = riasec.score_investigative || 0;
    document.getElementById('edit-artistic').value = riasec.score_artistic || 0;
    document.getElementById('edit-social').value = riasec.score_social || 0;
    document.getElementById('edit-enterprising').value = riasec.score_enterprising || 0;
    document.getElementById('edit-conventional').value = riasec.score_conventional || 0;
    document.getElementById('edit-note').value = riasec.note || '';
...
async function saveEdit() {
    ...
    const scores = {
        realistic: parseInt(document.getElementById('edit-realistic').value) || 0,
        investigative: parseInt(document.getElementById('edit-investigative').value) || 0,
        artistic: parseInt(document.getElementById('edit-artistic').value) || 0,
        social: parseInt(document.getElementById('edit-social').value) || 0,
        enterprising: parseInt(document.getElementById('edit-enterprising').value) || 0,
        conventional: parseInt(document.getElementById('edit-conventional').value) || 0
    };
    ...
    const payload = {
        ...
        answers: {},
        score_realistic: scores.realistic,
        score_investigative: scores.investigative,
        score_artistic: scores.artistic,
        score_social: scores.social,
        score_enterprising: scores.enterprising,
        score_conventional: scores.conventional,
        score_total: total,
        level_realistic: getLevel(scores.realistic, RIASEC_NORM.realistic),
        level_investigative: getLevel(scores.investigative, RIASEC_NORM.investigative),
        level_artistic: getLevel(scores.artistic, RIASEC_NORM.artistic),
        level_social: getLevel(scores.social, RIASEC_NORM.social),
        level_enterprising: getLevel(scores.enterprising, RIASEC_NORM.enterprising),
        level_conventional: getLevel(scores.conventional, RIASEC_NORM.conventional),
        level_total: getLevel(total, RIASEC_NORM.total),
        note: note,
        recorder_id: currentUser.id,
        completed_at: new Date().toISOString()
    };

    const { error } = await db.from('riasec_assessments').upsert(payload, { onConflict: 'student_id,academic_year,semester' });
    if (error) {
        Swal.fire('บันทึกไม่สำเร็จ', error.message, 'error');
        return;
    }
    await logUserAction(`แก้ไขผล RIASEC ของนักเรียน ${studentId}`, MODULE_ID);
...
async function deleteResult(studentId) {
    ...
    const { error } = await db.from('riasec_assessments').delete()
        .eq('student_id', studentId)
        .eq('academic_year', String(schoolInfo.current_academic_year))
        .eq('semester', String(schoolInfo.current_semester));
    ...
    await logUserAction(`ลบผล RIASEC ของนักเรียน ${studentId}`, MODULE_ID);
...
function exportExcel() {
    ...
    ws['!cols'] = [{ wch: 12 }, { wch: 8 }, { wch: 25 }, ...RIASEC_DIMENSIONS.map(() => ({ wch: 12 })), { wch: 12 }, { wch: 12 }];
...
async function printStudentPdf(studentId) {
    ...
    try {
        ...
        const { data: assessment, error: assessErr } = await db.from('riasec_assessments')
            .select('*')
            .eq('student_id', studentId)
            .eq('academic_year', String(schoolInfo.current_academic_year))
            .eq('semester', String(schoolInfo.current_semester))
        ...
        Swal.close();
        generateStudentPDFRIASEC(assessment, schoolName, academicYear, semester, adviser1, adviser2, schoolLogo, fullName, avatarUrl);
    } catch (err) {
        Swal.close();
...
async function handleSheetsImport() {
    ...
    try {
        ...
        const rows = text.trim().split('\n').slice(1).map(line => {
            ...
            return {
                'รหัสนักเรียน': cols[0],
                'Realistic': parseInt(cols[1]),
                'Investigative': parseInt(cols[2]),
                'Artistic': parseInt(cols[3]),
                'Social': parseInt(cols[4]),
                'Enterprising': parseInt(cols[5]),
                'Conventional': parseInt(cols[6])
            };
        });
        Swal.close();
        await processImportRows(rows);
...
async function processImportRows(rows) {
    ...
    for (const row of dataRows) {
        ...
        const scores = {};
        for (const [eng, key] of Object.entries(dimMap)) {
            scores[key] = parseInt(row[eng]) || 0;
        }
        const total = Object.values(scores).reduce((a, b) => a + b, 0);
        const payload = {
    ...
    await logUserAction(`นำเข้าข้อมูล RIASEC ${success} รายการ (ล้มเหลว ${fail} รายการ)`, MODULE_ID);
...
async function saveSettings() {
    ...
    const { error } = await db.from('riasec_settings').upsert({
        academic_year: schoolInfo.current_academic_year,
        semester: schoolInfo.current_semester,
        delay_seconds: delay,
        is_active: active
    ...
    if (error) {
        ...
        await logUserAction('บันทึกการตั้งค่าระบบ RIASEC', MODULE_ID);
...
async function loadCurrentAdmins() {
    try {
        ...
        if (html === '') {
            html = `<div class="text-center text-slate-400 py-8"><i class="fa-solid fa-user-slash text-3xl mb-2"></i><p>ยังไม่มีผู้ดูแลระบบ RIASEC</p></div>`;
        }
...
async function addModuleAdmin() {
    ...
    try {
        ...
        const { data: existing } = await db
            ...
            .eq('module_id', MODULE_ID)
            .maybeSingle();
        if (existing) return Swal.fire('ซ้ำซ้อน', 'บุคลากรนี้เป็นผู้ดูแล RIASEC อยู่แล้ว', 'info');
        ...
        await logUserAction(`แต่งตั้งผู้ดูแล RIASEC: ${personnel.prefix || ''}${personnel.first_name} ${personnel.last_name}`, MODULE_ID);
...
async function removeModuleAdmin(adminId, adminName) {
    ...
    const result = await Swal.fire({
        ...
        html: `คุณต้องการถอดถอน <strong>${adminName}</strong> จากการเป็นผู้ดูแลระบบ RIASEC ใช่หรือไม่?`,
        icon: 'warning',
        showCancelButton: true,
    ...
    try {
        ...
        if (error) throw error;
        await logUserAction(`ถอดถอนผู้ดูแล RIASEC: ${adminName}`, MODULE_ID);
...
async function openDimStudentList(dimKey) {
    const dim = RIASEC_DIMENSIONS.find(d => d.key === dimKey);
    if (!dim) return;
