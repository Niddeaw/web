 ...
 * 
 * แก้ไขครั้งล่าสุด: กำหนด group ข้อ 1 เป็น 2 (ข้อตรง) ตามเอกสาร
 */
...
// ── 52 ข้อคำถาม (group: 1 = ข้อกลับ, 2 = ข้อตรง) ───────────────────
const EQ_QUESTIONS_V2 = [
    // 1.1 ควบคุมตนเอง (ข้อ 1-6)
    { q: 1, dim: 'self_control', group: 2, text: 'เวลาโกรธไม่สบายใจ ฉันไม่รู้ว่าเกิดอะไรขึ้น' }, // แก้ไข: จาก group=1 เป็น group=2 (ข้อตรง)
...
function calcScoreV2(answers) {
    ...
    for (const q of EQ_QUESTIONS_V2) {
        ...
        let scored;
        // group = 2 => ข้อตรง (ตอบมาก = คะแนนสูง)
        // group = 1 => ข้อกลับ (ตอบมาก = คะแนนต่ำ)
        if (q.group === 2) {
            scored = raw;
        } else {
            scored = 5 - raw;
